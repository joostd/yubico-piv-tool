...
Both "selfsign" and "request-certificate" sign with the key in the slot, so they only work for a key that can sign.
X25519 and the ML-KEM algorithms are key agreement and key encapsulation algorithms respectively and cannot sign, so a
slot holding one of those goes straight from "generate" to "import-certificate" with a certificate issued elsewhere.
...
|-A, --algorithm   | | X | What algorithm to use to generate the key pair | RSA1024, RSA2048, RSA3072 (Requires YubiKey 5.7 or higher), RSA4096 (Requires YubiKey 5.7 or higher), ECCP256, ECCP384, ED25519 (Requires YubiKey 5.7 or higher), X25519 (Requires YubiKey 5.7 or higher), ML-DSA-44, ML-DSA-65, ML-DSA-87, ML-KEM-512, ML-KEM-768, ML-KEM-1024 (The six post-quantum algorithms require YubiKey 6.0 or higher) | RSA2048
...
==== Post-quantum key on slot 9a

Requires a YubiKey with firmware 6.0 or higher. ML-DSA public keys and the certificates around them are much larger
than their classical counterparts, so these examples write them to files instead of to the terminal.

    $ yubico-piv-tool -a generate -s 9a -A ML-DSA-87 -k -o pub.key
    Successfully generated a new private key.

    $ yubico-piv-tool -a verify-pin -a selfsign -s 9a -S '/CN=piv_auth/OU=test/O=example.com/' -i pub.key -o cert.pem
    Enter PIN:
    Successfully verified PIN.
    Successfully generated a new self signed certificate.
...
==== Post-quantum key encapsulation key on slot 9d

An ML-KEM key cannot sign, so the sequence stops after "generate" and the certificate, if the slot needs one at all,
has to come from somewhere else.

    $ yubico-piv-tool -a generate -s 9d -A ML-KEM-768 -k -o kem.pub
    Successfully generated a new private key.

The key is then used through link:test-decapsulation.adoc[test-decapsulate] or through the PKCS#11 module, see
link:../YKCS11/Functions_and_values.adoc[YKCS11 functions and values].
