...
=== Description
Tests decryption function. RSA keys are tested by decrypting a nonce, EC keys by performing an ECDH exchange.

Other key types are not covered. ED25519 and ML-DSA keys sign rather than decipher, and ML-KEM is a key
encapsulation mechanism with no plaintext of your choosing to encrypt, so it has its own action:
link:test-decapsulation.adoc[test-decapsulate].
